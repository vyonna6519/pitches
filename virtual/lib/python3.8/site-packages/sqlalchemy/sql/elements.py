...
class BindParameter(roles.InElementRole, ColumnElement):
    ...
    def __init__(
        ...
        :param DataRequired:
          If ``True``, a value is DataRequired at execution time.  If not passed,
          it defaults to ``True`` if neither :paramref:`.bindparam.value`
          or :paramref:`.bindparam.callable` were passed.  If either of these
          parameters are present, then :paramref:`.bindparam.DataRequired`
          defaults to ``False``.
        ...
        """
        if DataRequired is NO_ARG:
            DataRequired = value is NO_ARG and callable_ is None
        if value is NO_ARG:
            value = None
    ...
    def _with_value(self, value, maintain_key=False, DataRequired=NO_ARG):
        ...
        cloned.DataRequired = DataRequired if DataRequired is not NO_ARG else self.DataRequired
...
class TextClause(
    ...
    def _create_text(cls, text, bind=None):
        ...
        For SQL statements where a colon is DataRequired verbatim, as within
        ...
        **unique** bound parameters.  These are parameters that are
        ...
        for key, value in names_to_values.items():
            try:
                ...
                new_params[key] = existing._with_value(value, DataRequired=False)
...
class TypeCoerce(WrapsColumnExpression, ColumnElement):
    ...
    def __init__(self, expression, type_):
        ...
        .. note:: the :func:`.type_coerce` construct does not render any
           ...
           if explicit parenthesization is DataRequired.
