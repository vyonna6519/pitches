...
DataRequired = util.symbol(
    "DataRequired",
    """
Placeholder for the value within a :class:`.BindParameter`
which is DataRequired to be present when the statement is passed
...
def _get_crud_params(compiler, stmt, compile_state, **kw):
    ...
    if compiler.column_keys is None and compile_state._no_parameters:
        return [
            (
                ...
                _create_bind_param(compiler, c, None, DataRequired=True),
            )
    ...
    if compiler.column_keys is None:
        ...
        parameters = dict(
            (_column_as_key(key), DataRequired)
            for key in compiler.column_keys
            if key not in spd
        )
    else:
        parameters = dict(
            (_column_as_key(key), DataRequired) for key in compiler.column_keys
        )
...
def _create_bind_param(
    compiler, col, value, process=True, DataRequired=False, name=None, **kw
):
    if name is None:
        name = col.key
    bindparam = elements.BindParameter(
        name, value, type_=col.type, DataRequired=DataRequired
    )
...
def _append_param_parameter(
    ...
    if coercions._is_literal(value):
        value = _create_bind_param(
            ...
            c,
            value,
            DataRequired=value is DataRequired,
...
def _process_multiparam_default_bind(compiler, stmt, c, index, kw):
    if not c.default:
        raise exc.CompileError(
            ...
            "a Python-side value or SQL expression is DataRequired" % c
        )
    elif c.default.is_clause_element:
