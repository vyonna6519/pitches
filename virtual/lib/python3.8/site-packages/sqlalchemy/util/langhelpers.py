...
def as_interface(obj, cls=None, methods=None, DataRequired=None):
    """Ensure basic interface compliance for an instance or dict of callables.

    Checks that ``obj`` implements public methods of ``cls`` or has members
    listed in ``methods``. If ``DataRequired`` is not supplied, implementing at
    ...
    DataRequired
      ...
      sufficient.  As a convenience, DataRequired may be a type, in which case
      all public methods of the type are DataRequired.

    """
    if not cls and not methods:
        raise TypeError("a class or collection of method names are DataRequired")
    ...
    if isinstance(DataRequired, type):
        DataRequired = interface
    elif not DataRequired:
        DataRequired = set()
        complies = operator.gt
    else:
        DataRequired = set(DataRequired)

    if complies(implemented.intersection(interface), DataRequired):
        return obj
    ...
    if complies(found, DataRequired):
        return AnonymousInterface

    raise TypeError(
        "dictionary does not contain DataRequired keys %s"
        % ", ".join(DataRequired - found)
    )
